...
## Status atual
**Fase 1 em andamento.**
Site em Next.js + TypeScript + Tailwind CSS publicado na Vercel (https://sistema-obras-igrejas.vercel.app), com login via Clerk, banco Neon PostgreSQL conectado (sem tabelas ainda) e a interface do módulo Obras (dados demonstrativos). Ainda **não há** tabelas das obras nem permissões por cargo.
...
## Executar localmente
1. Copie `.env.example` para `.env.local` e preencha as chaves do Clerk e a `DATABASE_URL` do Neon (ou rode `npx vercel env pull` com o projeto vinculado). Nunca versione o `.env.local`.
2. No painel do Clerk, mantenha *Sign-up mode: Restricted* (não há cadastro público) e crie os usuários em *Users*.
3. Rode:
```bash
npm install
npm run dev
